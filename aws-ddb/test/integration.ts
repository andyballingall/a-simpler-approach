...
async function run() {
    ...
    let found = false;
    for (let i = 0; i < 20; i++) {
        const msgs = await sqsClient.send(new ReceiveMessageCommand({
        ...
        if (msgs.Messages && msgs.Messages.length > 0) {
            ...
            await sqsClient.send(new DeleteMessageCommand({ QueueUrl: SQS_QUEUE_URL, ReceiptHandle: message.ReceiptHandle }));
            found = true;
            break;
        }
