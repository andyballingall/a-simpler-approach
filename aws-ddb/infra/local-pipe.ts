...
async function main() {
    ...
    // 1. Get Stream ARN
    const table = await ddb.send(new DescribeTableCommand({ TableName: "entity-x" }));
    ...
    const openShard = shards.find(s => !s.SequenceNumberRange?.EndingSequenceNumber);
    if (!openShard) {
        console.log("No open shards found");
        return;
    }

    let iterator = (await streams.send(new GetShardIteratorCommand({
        StreamArn: streamArn,
    ...
    while (true) {
        ...
        const records = await streams.send(new GetRecordsCommand({ ShardIterator: iterator }));
        ...
        if (records.Records && records.Records.length > 0) {
            ...
            const entries = records.Records.map(r => {
                ...
                const payload = {
                    curr: r.dynamodb?.NewImage ? unmarshall(r.dynamodb.NewImage as any) : null,
                    prev: r.dynamodb?.OldImage ? unmarshall(r.dynamodb.OldImage as any) : null
                };
