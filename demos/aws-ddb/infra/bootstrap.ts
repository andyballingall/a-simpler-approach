...
async function main() {
  ...
  const tableDesc = await ddb.send(
  ...
  /*
    ...
    if ((e as any).name !== "EntityAlreadyExistsException") throw e;
  ...
  try {
    ...
    if ((e as Error).name !== "QueueAlreadyExists") throw e;
  }

  const qUrl =
    (await sqs.send(new GetQueueUrlCommand({ QueueName: "debug-queue" })))
      .QueueUrl ?? "";
  const qAttr = await sqs.send(
  ...
  try {
    ...
    console.log("Rule setup error", e);
  }

  await eb.send(new DescribeEventBusCommand({ Name: "cdc-bus" }));
  ...
  /*
    ...
    */
  console.log(
    "Skipping Pipe creation (requires Pro). Use 'bun run local:pipe' instead.",
  );
}
